...
Use `local_eval.py` to run quick self-play checks against a built-in opponent.

```bash
python local_eval.py --episodes 10 --opponent random
```

To render the final episode when your environment supports notebook rendering:

```bash
python local_eval.py --episodes 1 --opponent random --render
```

Run against a pool of opponents:

```bash
python local_eval.py --episodes 20 --opponents random,balanced,aggressive
```

Try parameter overrides without editing `main.py`:

```bash
python local_eval.py --episodes 20 --opponent random --params "{\"score_production_mult\":12,\"attack_min_score\":4.5}"
```

## Parameter Tuning

Use random search to find better heuristic weights:

```bash
python tune_params.py --trials 12 --episodes 6 --opponents random
```

This writes the best result to `best_params.json`. You can then evaluate it with:

```bash
python -c "import pathlib,subprocess; p=pathlib.Path('best_params.json').read_text(); subprocess.run(['python','local_eval.py','--episodes','30','--opponent','random','--params',p], check=True)"
```

## Submission Checklist

1. Keep the entrypoint in `main.py` and make sure it exposes `agent(obs)`.
2. Run `local_eval.py` to catch syntax or runtime issues before submitting.
3. Verify you have accepted the Kaggle rules and joined the competition.
4. Submit `main.py` as the single-file agent unless you intentionally bundle extra files.
5. Use the Kaggle submissions page to monitor the validation run and logs if a submission errors.
